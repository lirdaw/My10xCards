...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "primary persona"
      decision: "knowledge workers / professionals who retain domain knowledge"
    - topic: "pain moment"
      decision: "after reading — turning source text into cards"
    - topic: "product insight"
      decision: "one paste, one click — collapses the multi-step AI workflow"
    - topic: "MVP flow"
      decision: "sign up → paste text → review AI cards → accept/edit/reject → deck saved → review session"
    - topic: "timeline"
      decision: "max 3 weeks after-hours; hard deadline 2026-06-15"
    - topic: "product type"
      decision: "web-app"
    - topic: "user scale"
      decision: "medium (dozens to a hundred)"
  frs_drafted: 10
  quality_check_status: accepted
---

<!-- Shape session complete. Ready for /10x-prd. -->
...
The moment they reach for this product: they've just read an article, chapter, or document, and they want to convert it into a deck — right now, before the memory fades — but the blank Anki editor is a wall.

## Access Control

Multi-user web app. Access via email + password login.

- Sign-up: email + password (standard registration flow).
...
2. User pastes source text into the generation input.
3. AI generates a set of flashcard drafts.
4. User reviews drafts — accepting, editing, or rejecting each.
5. Accepted cards are saved to the user's deck.
6. User starts a spaced repetition review session from the deck.
...
- Generation failure (LLM error) surfaces an explicit error and allows the user to retry or switch to manual creation.
...
The spaced repetition scheduling (when each card surfaces for review) is handled by an integrated, off-the-shelf SRS algorithm. The product does not own the scheduling decision; it owns the generation quality decision. SRS is a dependency, not a domain rule.
...
- **No custom SRS algorithm**: the product uses an off-the-shelf spaced repetition library (e.g., SM-2 or FSRS). Building a proprietary algorithm is explicitly out of scope for MVP and beyond.
...
Result: accepted — all required elements present.
